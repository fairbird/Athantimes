...
from Plugins.Extensions.AthanTimes.outils.compat import compat_urlopen
...
class WeatherList(Screen):
    ...
    def __init__(self, session, Location):
        ...
        skin = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Skin/WeatherList.xml'
    ...
    def go(self):
        ...
        if Location_id and Region_id:
            iLocation = open('/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Config/Location_id', 'w')
            iLocation.write(str(Location_id))
            iLocation.close()
            iRegion = open('/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Config/Region_id', 'w')
...
class WeatherSearch(Screen):
    ...
    def __init__(self, session, city):
        self.session = session
        self.EnterLocation = city
        self.iTimer = eTimer()
        self.iTimer.stop()
        try:  # Edit By RAED For DreamOS
                self.iTimer.callback.append(self.KeyBoard)
        except:
                self.iTimer_conn = self.iTimer.timeout.connect(self.KeyBoard)
