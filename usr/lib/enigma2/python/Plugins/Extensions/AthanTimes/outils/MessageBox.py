...
class MessageBox(Screen):
    ...
    def __init__(self, session, text, type=TYPE_YESNO, timeout=-1, close_on_any_key=False, default=True, enable_input=True, msgBoxID=None, picon=None, simple=False, list=[], timeout_default=None):
        ...
        self.VRS = self.Version()
        self.title = self.type < self.TYPE_MESSAGE and ['   ســؤال' + '   AthanTimes' + str(self.VRS),
         '   للإعلام' + '   AthanTimes' + str(self.VRS),
         '   حــذاري' + '   AthanTimes' + str(self.VRS),
         '   خــطأ' + '   AthanTimes' + str(self.VRS)][self.type] or 'Message'
        if type == self.TYPE_YESNO:
            if list:
                ...
                self.list = [
                 (
                  _('نعم'), True), (_('لا'), False)]
            else:
                self.list = [
                 (
                  _('لا'), False), (_('نعم'), True)]
        else:
            self.list = []
    ...
    def Version(self):
        Vers = open('/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/Version', 'r')
        VR = Vers.readlines()
        for line in VR:
            if 'Version' in line:
                self.VRS = line.split('=')[1]
