# -*- coding: utf-8 -*-
from Plugins.Extensions.AthanTimes.plugin import ScreenPrayerTimes_Show
...
from Plugins.Extensions.AthanTimes.outils.compat import compat_urlopen, compat_Request, compat_HTTPError, compat_URLError
...
class MeteoMain(Screen):
    ...
    def __init__(self, session, city):
        path = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Skin/WeatherCompact.xml'
        ...
        self['5day'] = Label(_('5 Day Weather Forecast_\xd8\xa3\xd8\xad\xd9\x88\xd8\xa7\xd9\x84 \xd8\xa7\xd9\x84\xd8\xb7\xd9\x82\xd8\xb3 \xd9\x84\xd9\x80 5 \xd8\xa3\xd9\x8a\xd8\xa7\xd9\x85'))
        self['lab1b'] = Label('')
        self['lab2'] = Label('')
        self['lab3'] = Label('')
        self['lab4'] = Label('')
        ...
        self['daydate3'] = Label('')
        self['daydate4'] = Label('')
        self['actions'] = ActionMap(['WizardActions', 'ColorActions', 'MenuActions'], {'green': self.key_Green,
         'back': self.END,
         'red': self.END,
         'ok': self.END})
    ...
    def updateInfo(self):
        from Plugins.Extensions.AthanTimes.outils.Utils import Change_TXTDay_2
        myurl = self.get_Url()
        req = compat_Request(myurl)
        try:
            handler = compat_urlopen(req)
            ...
            if dom:
                ...
                self['lab1b'].setText(_('Yahoo Weather_\xd8\xa7\xd9\x84\xd8\xb7\xd9\x82\xd8\xb3'))
                city = '%s' % str(weather_data['location']['city'])
                idcity = self.get_IdCity()
                self['lab2'].setText(city + ':' + str(idcity))
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon/%s.png' % str(weather_data['condition']['code'])
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon/%s.png' % str(weather_data['forecasts'][0]['code'])
                myicon = self.checkIcon(icon)
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon/%s.png' % str(weather_data['forecasts'][1]['code'])
                myicon = self.checkIcon(icon)
                png = loadPic(myicon, 81, 59, 0, 0, 0, 0)
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon/%s.png' % str(weather_data['forecasts'][2]['code'])
                myicon = self.checkIcon(icon)
                png = loadPic(myicon, 81, 59, 0, 0, 0, 0)
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon/%s.png' % str(weather_data['forecasts'][3]['code'])
                myicon = self.checkIcon(icon)
                png = loadPic(myicon, 81, 59, 0, 0, 0, 0)
                ...
                icon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon/%s.png' % str(weather_data['forecasts'][4]['code'])
                myicon = self.checkIcon(icon)
                png = loadPic(myicon, 81, 59, 0, 0, 0, 0)
                ...
                myicon = '/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Skin/1color.png'
    ...
    def checkIcon(self, localfile):
        if fileExists(localfile):
            ...
            url = localfile.replace('/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Icon', 'http://www.mysite.net/weapic/weabig')
            handler = compat_urlopen(url)
            if handler:
    ...
    def get_Url(self):
        ...
        url3 = '1253079'
        if fileExists('/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Config/Location_id'):
            url2 = open('/usr/lib/enigma2/python/Plugins/Extensions/AthanTimes/outils/YahooWeather/Config/Location_id').read()
            if url2 == 'City Not Found':
                url2 = url3
            else:
                url2 = url2
    ...
    def key_Green(self):
        from Plugins.Extensions.AthanTimes.outils.YahooWeather.Search_Id import WeatherSearch
        self.session.open(WeatherSearch, self.city)
